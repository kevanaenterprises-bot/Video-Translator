...
export function useTranslation(sessionId: string, localStream?: MediaStream | null): UseTranslationReturn {
  ...
  const startSpeechRecognition = useCallback(async () => {
    // ALWAYS read from refs — prevents stale closures after language change
    const src = yourLanguageRef.current;
    const tgt = partnerLanguageRef.current;
    ...
    try {
      ...
      // NOTE: useSpeechRecognition now handles its own restart on fatal errors only.
      // Only restart here on truly fatal errors (not 'no-speech' or 'aborted')
      console.log(`👂 Starting continuous listening for ${src.name}...`);
      startListening(speechCode).catch(error => {
        console.log("Speech recognition fatal error:", error?.message || error);
        ...
        // 'aborted' = user stopped — no restart needed
        // 'no-speech' is now handled silently in useSpeechRecognition
      });
